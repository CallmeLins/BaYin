  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: tokens.barBg.withValues(alpha: 0.85),
        border: Border(
          top: BorderSide(color: tokens.separatorColor, width: 1),
        ),
      ),
      child: SizedBox(
        height: height,
        ...
        return SafeArea(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border(
                top: BorderSide(
                  color: FlatColors.border(brightness).withValues(alpha: 0.35),
                  width: 1,
                ),
              ),
            ),
...
class _QueueRowState extends State<_QueueRow> {
  ...
  Widget build(BuildContext context) {
    final bg = widget.active
        ? widget.activeColor
        : (_hovering ? widget.normalColor : Colors.transparent);
...
class _ProgressScrubBar extends StatefulWidget {
  ...
  @override
  State<_ProgressScrubBar> createState() => _ProgressScrubBarState();
}

class _ProgressScrubBarState extends State<_ProgressScrubBar> {
  bool _hovering = false;
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final showTrack = _hovering || _interacting;
        return MouseRegion(
          onEnter: (_) => setState(() => _hovering = true),
          onExit: (_) => setState(() => _hovering = false),
          child: GestureDetector(
            ...
            onTapDown: (d) {
              ...
              widget.onSeek(calc(d.globalPosition, context));
            },
            onTapUp: (_) => setState(() => _interacting = false),
            onTapCancel: () => setState(() => _interacting = false),
            ...
            onHorizontalDragUpdate: (d) =>
                widget.onSeek(calc(d.globalPosition, context)),
            onHorizontalDragEnd: (_) => setState(() => _interacting = false),
                ...
                child: Stack(
                  children: [
                    ...
                    FractionallySizedBox(
                      widthFactor: widget.progressPercent,
                      child: Container(
                        height: 2,
                        decoration: BoxDecoration(
...
class _TransportCluster extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Tauri/Web parity: white in light mode, translucent white in dark mode.
    final playBg = isDark ? Colors.white.withValues(alpha: 0.10) : Colors.white;
...
class _PlayCircleButtonState extends State<_PlayCircleButton> {
  ...
  Widget build(BuildContext context) {
    final scale = _pressing
        ? 0.95
        : (_hovering && widget.enabled ? 1.05 : 1.0);
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        onTapCancel: widget.enabled ? () => setState(() => _pressing = false) : null,
        child: AnimatedScale(
          duration: const Duration(milliseconds: 150),
          curve: Curves.easeOut,
          scale: scale,
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          height: 36,
          decoration: BoxDecoration(
            color: _hovering && widget.onPressed != null
                ? hoverBg
                : Colors.transparent,
            borderRadius: BorderRadius.circular(FlatRadius.circle),
          ),
