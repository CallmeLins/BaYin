...
class MediaPlaybackService : Service() {
    ...
    override fun onCreate() {
        ...
        // HyperOS 2.0+ aggressively kills background services before they can
        // call startForeground().  Promote to foreground immediately with a
        // placeholder notification so the system preserves the service.
        val placeholder = buildNotification(false, false, false, "BaYin", "")
    ...
    private fun updateFromRust() {
        ...
        if (id.isEmpty()) {
            ...
            // Keep the foreground service alive (HyperOS 2.0 kills background services
            // too quickly).  Just update with a minimal notification.
            if (isForeground) {
                val placeholder = buildNotification(false, false, false, "BaYin", "")
                nm.notify(NOTIFICATION_ID, placeholder)
            }
            return
        }
